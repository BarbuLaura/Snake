...
function checkGameRight(direction){
...
function checkGameLeft(direction){
...
function checkGameUp(direction){
...
function checkGameDown(direction){
  if( direction === 16 && (currentSnake[0] % nrSquares)  === currentSnake[0] && 
  currentSnake[0] >= boardSize * (boardSize - 1)){
    return 1;
  }
  return 0;
}  

function checkGameInnerBody(){
...
function moveStatus(){
  if(checkGameRight(direction) === 1 || checkGameLeft(direction) === 1 || 
  checkGameDown(direction) === 1 || checkGameUp(direction) === 1 || 
  checkGameInnerBody() === 1){
